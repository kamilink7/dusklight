...
#if TARGET_PC
        if (isExecuting()) {
            follow_button_xy(i_no, true, !dMeter2Info_isUseButton(i_no == 0 ? 4 : (i_no == 1 ? 8 : 0x800)), getFishingType() != 0, mpItemXY(i_no)->isVisible());
        } else
#endif
        {
            f32 var_f30 = 1.0f;
            ...
            if (i_no == 0) {
                ...
                if (DUSK_IF_ELSE(s_follow_xy[i_no].dim, !dMeter2Info_isUseButton(0x800))) {
                    var_r26_3 = 0;
                }
            }
